...
func (s *HTTPServer) platformErr(c *gin.Context, err error) {
	...
	if errors.Is(err, platform.ErrNotConfigured) {
		response.Fail(c, http.StatusServiceUnavailable, response.CodeErr,
			"平台接入未配置：请在 configs/config.yaml 的 platform 段补全凭证")
		return
	}
